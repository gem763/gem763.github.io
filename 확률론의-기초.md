...
결과를 예측할 수 없는 실험을 같은 조건으로 무작위 반복할 때, 이를 [**확률실험**(Random experiment)](https://en.wikipedia.org/wiki/Experiment_(probability_theory)) 또는 확률시행(Random trial)이라고 한다.  수학적으로 확률이란, **확률실험의 결과에 의미있는 숫자를 할당**하는 행위를 말한다. 우선 `표본공간`과 `사건공간`의 개념을 이해해보자. 
