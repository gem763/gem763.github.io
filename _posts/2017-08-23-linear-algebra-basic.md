...
## Norm
Norm에는 Vector norm과 Matrix norm이 있다. 각각에 대해서 알아보자. 
...
\begin{aligned}
\footnotesize\it{\text{Triangle inequality}} ~~~&\Vert \mathbf{x}+\mathbf{y} \Vert \le \Vert \mathbf{x} \Vert + \Vert \mathbf{y} \Vert \\
\footnotesize\it{\text{Absolutely homogeneous}} ~~~&\Vert \alpha \mathbf{x} \Vert = |\alpha| \Vert \mathbf{x} \Vert \\
\footnotesize\it{\text{Positive definite}} ~~~&\Vert \mathbf{x} \Vert = 0 ~\Longleftrightarrow~ \mathbf{x}=\mathbf{0}
\end{aligned}
</script></div>
...
<br/><span><script type="math/tex">p</script></span> 값에 따라 아래와 같이 유도되며, 각자 고유의 이름이 있다. 특히 <span><script type="math/tex">p=2</script></span> 인 Euclidean norm을 가장 많이 쓰는 편이다. 
...
<div class="math"><script type="math/tex; mode=display">
\begin{aligned}
\footnotesize\it{\text{Triangle inequality}} ~~~&\Vert \mathbf{A}+\mathbf{B} \Vert \le \Vert \mathbf{A} \Vert + \Vert \mathbf{B} \Vert \\
\footnotesize\it{\text{Absolutely homogeneous}} ~~~&\Vert \alpha \mathbf{A} \Vert = |\alpha| \Vert \mathbf{A} \Vert \\
\footnotesize\it{\text{Positive definite}} ~~~&\Vert \mathbf{A} \Vert = 0 ~\Longleftrightarrow~ \mathbf{A}=\mathbf{O}
\end{aligned}
</script></div>
...
<div class="math"><script type="math/tex; mode=display">
\Vert \mathbf{A} \Vert_{p} \equiv \sup_{\mathbf{x} \not= 0} \frac{\Vert \mathbf{A} \mathbf{x} \Vert_p}{\Vert \mathbf{x} \Vert_p}
</script></div>
...
## Trace
Trace는 **정방행렬의 대각성분을 모두 합산**한 값이다. 즉 행렬 <span><script type="math/tex">\mathbf{A} = [a_{ij}] \in \mathbb{R}^{n \times n}</script></span>에 대해서 다음과 같이 정의된다. 
...
행렬식이라고도 한다. 정방행렬 <span><script type="math/tex">\mathbf{A} = [a_{ij}] \in \mathbb{R}^{n \times n}</script></span>에 대해, 다음과 같이 재귀적(Recurisve)인 방식으로 정의되는데, 이를 Cofactor expansion이라고 부른다. 임의의 <span><script type="math/tex">k</script></span>-행 또는 임의의 <span><script type="math/tex">k</script></span>-열에 대해서, 
...
즉 <span><script type="math/tex">\mathbf{A}</script></span>와 코팩터의 각기 다른 열에서 원소를 선택하여 서로 곱하면 그 값은 0이 되는 것이다. 임의의 (<span><script type="math/tex">h,k</script></span>)-행에 대해서도 마찬가지이다. 이는 아래에서, <span><script type="math/tex">\mathbf{A}</script></span>의 역행렬을 유도하는 과정에서 사용하게 되는 주요 성질이다. 증명은 [여기](https://proofwiki.org/wiki/Matrix_Product_with_Adjugate_Matrix)를 참고. 


<br/>

## 역행렬
정방행렬 <span><script type="math/tex">\mathbf{A} \in \mathbb{R}^{n \times n}</script></span>에 대하여 다음의 관계를 만족하는 행렬 <span><script type="math/tex">\mathbf{B} \in \mathbb{R}^{n \times n}</script></span> 가 존재할 때, 행렬 <span><script type="math/tex">\mathbf{A}</script></span>가 **Invertible** (가역적) 또는 **Non-singular** 하다고 말한다. 

<div class="math"><script type="math/tex; mode=display">
\mathbf{A} \mathbf{B} = \mathbf{B} \mathbf{A} = \mathbf{I} 
</script></div>

행렬 <span><script type="math/tex">\mathbf{A}</script></span>가 Invertible한 경우, <span><script type="math/tex">\mathbf{B}</script></span>는 **유일하게 존재**하게 된다. 이를  **<span><script type="math/tex">\mathbf{A}</script></span>의 역행렬**이라고 부르고, <span><script type="math/tex">\mathbf{A}^{-1}</script></span>로 표기한다. 

역행렬을 좀더 명시적으로 표현할 수도 있다. 우선 행렬 <span><script type="math/tex">\mathbf{A}</script></span>에 대한 **[Adjugate matrix](https://en.wikipedia.org/wiki/Adjugate_matrix)** <span><script type="math/tex">\text{adj}(\mathbf{A}) \in \mathbf{R}^{n \times n}</script></span>를 다음과 같이 정의해보자. 
...
따라서 <span><script type="math/tex">\mathbf{A}</script></span>의 역행렬은 다음과 같이 명시적으로 표현된다. <span><script type="math/tex">\det \mathbf{A}=0</script></span> 인 경우 역행렬이 정의되지 않음을 알 수 있다. 
<div class="math"><script type="math/tex; mode=display">
...
역행렬은 여러가지 유용한 성질을 가지고 있다. 역행렬이 존재하는 행렬 <span><script type="math/tex">\mathbf{A}, \mathbf{B} \in \mathbf{R}^{n \times n}</script></span>와 임의의 벡터 <span><script type="math/tex">\mathbf{x} \in \mathbb{R}^n</script></span> 및 실수 <span><script type="math/tex">\alpha</script></span> (<span><script type="math/tex">\not= 0</script></span>)에 대해서, 
...
## 행렬의 부호
행렬의 크기를 측정할 수 있는 것과 마찬가지로, 행렬의 부호 역시 정의내릴 수 있다. 
