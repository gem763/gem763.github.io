...
### Sortino
**투자전략의 Downside risk 대비 CAGR**을 [Sortino](https://en.wikipedia.org/wiki/Sortino_ratio)라고 한다. 투자전략이 감내하는 손실위험에 비해, 목표수익률 대비 투자수익률이 얼마나 좋았는지를 나타낸다. 이 블로그에서는 목표수익률 <span><script type="math/tex">\theta = 0</script></span> 을 선호한다. 
...
![D(T)=\max \left\{0,\max _{{t\in (0,T)}}X(t)-X(T)\right\}](https://wikimedia.org/api/rest_v1/media/math/render/svg/28986206f995a7330176e801d7bb80cd3b114ede)

The  **maximum drawdown**  (MDD) up to time  {\displaystyle T}![T](https://wikimedia.org/api/rest_v1/media/math/render/svg/ec7200acd984a1d3a3d7dc455e262fbe54f7f6e0)  is the maximum of the Drawdown over the history of the variable. More formally,

{\displaystyle {\text{MDD}}(T)=\max _{\tau \in (0,T)}\left[\max _{t\in (0,\tau )}X(t)-X(\tau )\right]}![{\text{MDD}}(T)=\max _{{\tau \in (0,T)}}\left[\max _{{t\in (0,\tau )}}X(t)-X(\tau )\right]](https://wikimedia.org/api/rest_v1/media/math/render/svg/0d2b217df2d2894731f707965398b0ab901459d7)
