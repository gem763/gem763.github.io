...
상수인 <span><script type="math/tex">\alpha, \beta \in \mathbb{R}</script></span>에 대해, <span><script type="math/tex">R_p</script></span>는 다음과 같이 표현할 수 있으며, 
...
현실에서는 위 수식에 포함되어 있는 공분산과 분산을 추정하는 과정이 추가적으로 필요하다. 가장 단순한 방법으로는, 샘플 공분산과 샘플 분산으로 해당 값들을 추정하는 것이다.
...
> <big>**회귀계수와의 비교**</big>
> 
> 보통 우리가 <span><script type="math/tex">\beta</script></span>를 구할 때, BM의 과거 수익률과 투자전략 과거 수익률간의 **회귀계수**(Regression
> coefficient)를 측정하는 경우가 많다. 그렇다면 **샘플링을 통해 추정한 <span><script type="math/tex">\hat\beta</script></span>와는 어떤 관계**를
> 가지고 있을까?
> 
...
> </script></div>
> 
> 따라서 회귀계수 <span><script type="math/tex">\beta</script></span>는 다음과 같이 도출된다. [^min_err]
> 
> <div class="math"><script type="math/tex; mode=display">
> \begin{aligned}
