...
수식전개의 편의를 위해, 각 수익률 변수의 기대값으로 중심축을 이동하면 <span><script type="math/tex">\alpha</script></span> 를 제거할 수 있다. 즉
...
결과적으로 **선형회귀 방정식은 <span><script type="math/tex">(\mathbf{E}[R_b], \mathbf{E}[R_p])</script></span> 점을 지나가게 된다**. 

이제 <span><script type="math/tex">\beta</script></span>를 추정해보자. **최소제곱법은 오차항 <span><script type="math/tex">\epsilon</script></span>의 크기를 최소화 하는 방향으로 <span><script type="math/tex">\beta</script></span>를 찾는 기법**이다. <span><script type="math/tex">Z_p</script></span>의 샘플벡터를 <span><script type="math/tex">\mathbf{z}_p \in \mathbb{R}^m</script></span>, <span><script type="math/tex">Z_b</script></span>의 샘플벡터를 <span><script type="math/tex">\mathbf{z}_b \in \mathbb{R}^m</script></span> 라고 하고, 오차항 벡터 <span><script type="math/tex">\mathbf{e} \in \mathbb{R}^m</script></span> 를 다음과 같이 정의하자. 
...
> 
> 위에서 최소제곱법으로 산출한 <span><script type="math/tex">\beta</script></span> 값을 **통계지표 측면**에서 나타낼 수도 있다. 
...
> &= \rho \frac{\sigma_p}{\sigma_b}
> \end{aligned}
> </script></div>
>
> 여기서 <span><script type="math/tex">\rho \equiv \frac{\mathbf{Cov}[R_b, R_p]}{\sqrt{\mathbf{Var}[R_b] \mathbf{Var}[R_p]}}</script></span> 는 투자전략의 수익률과 BM 수익률 간의 상관계수, <span><script type="math/tex">\sigma_p</script></span>는 투자전략의 샘플 표준편차, <span><script type="math/tex">\sigma_b</script></span>는 BM의 샘플 표준편차를 의미한다. 
...
> 
> 실험 관찰자의 사전지식 등으로 인해, **어떤 선형관계가 원점을 지날 것이라고 강한 확신**을 하는 경우가 있다. 이럴 때에는 <span><script type="math/tex">\alpha=0</script></span> 으로 놓고, 회귀방정식의 형태를 다음과 같이 가정한다. 
> <div class="math"><script type="math/tex; mode=display">
> Y =  \beta X + \epsilon
> </script></div>
> 
...
## Consistency
**투자성과의 지속성**을 평가하기 위한 지표이다. 투자성과의 **로그 누적수익률**에 선형회귀분석을 시행한 후, [R-Square](https://en.wikipedia.org/wiki/Coefficient_of_determination)를 취한다. 결과적으로는 **로그 누적수익률이 얼마나 선형(Linear)에 가까운 지를 측정**하게 된다. 

<center><i>개인적인 필요에 의해, 이 블로그에서 만든 개념임을 미리 알린다</i></center>

투자성과의 로그 누적수익률 변수를 <span><script type="math/tex">S \in \mathbb{R}</script></span> 라고 두자. 예를들어 과거의 투자성과에서 임의의 연속적인 시간벡터 <span><script type="math/tex">\mathbf{t} \in \mathbb{R}^m</script></span>에 대한 잔고액(또는 가격)을 추출하고 이를 벡터 <span><script type="math/tex">\mathbf{p} \in \mathbb{R}^m</script></span> 라고 하면, 
...
\Downarrow \\[5pt]
\mathbf{s} = \textstyle (0, \ln \frac{p_1}{p_0}, \ln \frac{p_2}{p_0}, \cdots, \ln \frac{p_{m-1}}{p_0}) = (s_0, s_1, s_2, \cdots, s_{m-1})
\end{matrix}
</script></div>

로그 누적수익률 <span><script type="math/tex">S</script></span>로 부터의 샘플데이터 벡터 <span><script type="math/tex">\mathbf{s} \in \mathbb{R}^m</script></span> 는 위의 형태가 된다. 정의에 의해, 샘플벡터 <span><script type="math/tex">\mathbf{s}</script></span> 는 늘 0에서부터 시작한다. 즉 **회귀모형이 원점을 지날 것이라고 사전적으로 알고 있다**. 

로그 누적수익률 변수 <span><script type="math/tex">S \in \mathbb{R}</script></span> 가 시간변수 <span><script type="math/tex">T</script></span>와 다음의 선형관계를 가지고 있다고 가정하자. 

<div class="math"><script type="math/tex; mode=display">
S = \beta T + \epsilon
</script></div>

여기서 추정한 <span><script type="math/tex">\hat\beta</script></span>을 이용하면, <span><script type="math/tex">S</script></span>의 추정값 <span><script type="math/tex">\hat S</script></span>에 대해서, 

<div class="math"><script type="math/tex; mode=display">
\hat S = \hat\beta T ~\Longrightarrow~ S = \hat S + \hat \epsilon
</script></div>

따라서 종속변수인 <span><script type="math/tex">S</script></span>의 분산은 다음과 같이 분해된다. 

<div class="math"><script type="math/tex; mode=display">
\begin{aligned}
\mathbf{Var}[S] &= \mathbf{Var}[\hat S + \hat\epsilon] \\
&= \mathbf{Var}[\hat S] + \mathbf{Var}[\hat\epsilon] + 2 \mathbf{Cov} [\hat S, \hat\epsilon]
\end{aligned}
</script></div>

따라서 Consistency <span><script type="math/tex">\mathbf{C}</script></span>는 <span><script type="math/tex">\mathbf{R}^2</script></span> (R-Square) 의 정의[^rsquare]에 의해  다음과 같이 나타낼 수 있다.  

[^rsquare]: 종속변수의 분산 중 회귀모형으로 설명이 불가능한 부분을 제외한 비율. **결정계수** (Coefficient of determination) 또는 **설명력**이라고도 한다. 


<div class="math"><script type="math/tex; mode=display">
\mathbf{C} = \mathbf{R}^2 \equiv 1 - \frac{\mathbf{Var}[\hat\epsilon]}{\mathbf{Var}[S]}
</script></div>


실제 계산은 통상 샘플분산을 통하는 경우가 많다. 다음의 각 샘플데이터에 대해서, 

* 종속변수 샘플 <span><script type="math/tex">\mathbf{s} = [s_i] \in \mathbb{R}^m</script></span>, <span><script type="math/tex">\bar s = \mathbf{E}[S] = \frac{1}{m} \sum_i s_i</script></span>
* 추정값 샘플 <span><script type="math/tex">\hat{\mathbf{s}} = [\hat s_i] \in \mathbb{R}^m</script></span>, <span><script type="math/tex">\bar {\hat s} = \mathbf{E}[\hat S] = \mathbf{E}[S] = \bar s</script></span>
* 오차항 샘플 <span><script type="math/tex">\hat\mathbf{e} \equiv \mathbf{s} - \hat{\mathbf{s}} = [\hat e_i]</script></span>, <span><script type="math/tex">\bar {\hat e} = \mathbf{E}[\hat\epsilon] = 0</script></span> 

다음의 세 가지를 새로 정의하자. 

* **Total Sum of Squares**
<div class="math"><script type="math/tex; mode=display">
\mathbf{TSS} \equiv \sum_i (s_i - \bar s)^2 = m \mathbf{Var}[S]
</script></div>

* **Explained Sum of Squares**
<div class="math"><script type="math/tex; mode=display">
\mathbf{ESS} \equiv \sum_i (\hat s_i - \bar s)^2 = \sum_i (\hat s_i - \bar {\hat s})^2 = m \mathbf{Var}[\hat S]
</script></div>

* **Residual Sum of Squares**
<div class="math"><script type="math/tex; mode=display">
\mathbf{RSS} \equiv \sum_i \hat e_i^2 = m \mathbf{Var}[\hat \epsilon]
</script></div>

따라서 정의에 이해, 이들 간에는 다음의 관계가 성립한다. 

<div class="math"><script type="math/tex; mode=display">
\underbrace{\mathbf{Var}[S]}_{\normalsize\frac{\mathbf{TSS}}{m}} = \underbrace{\mathbf{Var}[\hat S]}_{\normalsize\frac{\mathbf{ESS}}{m}} + \underbrace{\mathbf{Var}[\hat\epsilon]}_{\normalsize\frac{\mathbf{RSS}}{m}} + 2 \mathbf{Cov} [\hat S, \hat\epsilon]
</script></div>


따라서 Consistency 는 다음과 같이 계산된다. 

<div class="math"><script type="math/tex; mode=display">
\mathbf{C} =  1 - \frac{\mathbf{RSS}}{\mathbf{TSS}}
</script></div>
