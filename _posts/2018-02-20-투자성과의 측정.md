...
*참고로, Consistency는 이 블로그에서의 필요에 의해 만들어진 개념이며, 학문적인 배경이 전혀 없는 지표이니 주의하기 바란다.* 
...
정의에 의해, 로그정규가격 샘플벡터의 첫번째 샘플 <span><script type="math/tex">q_0=0</script></span> 이다. 즉 **회귀방정식이 원점을 지날 것이라고 사전적으로 알고 있는 것이다**. 따라서 로그정규가격 변수 <span><script type="math/tex">Q</script></span> 와 시간변수 <span><script type="math/tex">T</script></span>간의 선형방정식이 원점을 지난다고 가정할 수 있다. 

<div class="math"><script type="math/tex; mode=display">
Q = \beta T + \epsilon
</script></div>

최소자승법을 통해 추정한 <span><script type="math/tex">\hat\beta</script></span>을 이용하면, <span><script type="math/tex">Q</script></span>의 추정값 <span><script type="math/tex">\hat Q</script></span>을 알 수 있다. 따라서, 
...
> 
> <div class="math"><script type="math/tex; mode=display">
\underbrace{\mathbf{Var}[Q]}_{\normalsize\frac{\mathbf{TSS}}{m}} = \underbrace{\mathbf{Var}[\hat Q]}_{\normalsize\frac{\mathbf{ESS}}{m}} + \underbrace{\mathbf{Var}[\hat\epsilon]}_{\normalsize\frac{\mathbf{RSS}}{m}} + 2 \mathbf{Cov} [\hat Q, \hat\epsilon]
</script></div>
