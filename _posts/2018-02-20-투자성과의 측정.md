...
## 추적오차
**BM 대비 상대적인 변동성**을 계산한 값을 추적오차(Tracking error)라고 한다. 예를들어, 한국시장 전체에 투자하기 위해 코스피 인덱스 펀드에 가입했다고 가정해보자. 그런데 어떤 펀드는 코스피 인덱스와 매일 1% 이상씩 **위아래로 차이**가 나고, 또 다른 펀드는 매일 0.1% 정도만 차이가 난다고 하면, 과연 어느 펀드에 가입할까? 후자일 가능성이 높은 데, 이러한 경우를 추적오차가 작다고 얘기한다. [^te]
...
### Sharpe
**투자전략의 변동성 대비 CAGR**을 Sharpe 라고 한다. 즉 투자전략 자체의 변동성에 비해 투자전략 자체의 수익률[^shp_rtn]이 얼마나 좋았는지를 나타낸다. 가장 많이 사용되는 위험조정수익률 지표이다. 
...
### IR
**투자전략의 추적오차 대비 초과수익률**을 IR (Information Ratio)라고 한다. 정보비율이라고도 부른다. 즉 투자전략의 BM대비 변동성에 비해 얼마나 초과수익률을 달성했는지를 나타낸다.
...
이제 <span><script type="math/tex">R_p</script></span>는 다음과 같이 표현할 수 있으며, 

<div class="math"><script type="math/tex; mode=display">
R_p = \beta R_b + \epsilon
</script></div>

양변에 <span><script type="math/tex">\mathbf{Cov}[R_b, \cdot]</script></span> 를 취하면 다음을 얻는다. 

<div class="math"><script type="math/tex; mode=display">
\begin{aligned}
\mathbf{Cov}[R_b, R_p] &= \mathbf{Cov}[R_b, \beta R_b + \epsilon] \\
...
> <big>**회귀계수**</big>
> 
> 현실적으로는 BM의 과거 수익률과 투자전략 과거 수익률간의 **회귀계수**(Regression coefficient)로 <span><script type="math/tex">\beta</script></span>를 측정하는 경우가 많다. 각각의 과거 수익률 벡터들에 대해서, 
> * BM의 과거 수익률 벡터 <span><script type="math/tex">\mathbf{r}_b \in \mathbb{R}^m</script></span>
> * 투자전략의 과거수익률 벡터 <span><script type="math/tex">\mathbf{r}_p \in \mathbb{R}^m</script></span>
> 
> [최소자승법 (OLS: Ordinary Least Squares)](https://en.wikipedia.org/wiki/Ordinary_least_squares)을 쓰면, 회귀계수 <span><script type="math/tex">\beta^*</script></span>는 다음과 같이 구해진다. 
> <div class="math"><script type="math/tex; mode=display">
> \beta^* = (\mathbf{r}_b^\mathsf{T}\mathbf{r}_b)^{-1} \mathbf{r}_b^\mathsf{T} \mathbf{r}_p
> </script></div>
> 그렇다면 바로 위에서 도출한 <span><script type="math/tex">\beta</script></span>와 회귀계수 <span><script type="math/tex">\beta^*</script></span>는 어떤 차이가 있을까? <span><script type="math/tex">R_p</script></span>와 <span><script type="math/tex">R_b</script></span> 사이의 관계를 통해 우리는 다음을 알 수 있다. 
> <div class="math"><script type="math/tex; mode=display">
> \begin{aligned}
> \mathbf{E}[R_p] &= \beta \mathbf{E}[R_b] \\
> R_p + \mathbf{E}[R_p] &= \beta R_b + \epsilon + \mathbf{E}[R_p]  \\
> &= \beta (R_b + \mathbf{E}[R_b]) + \epsilon
> \end{aligned}
> </script></div>
> 즉, 각 수익률 확률변수에서 기대수익률 <span><script type="math/tex">\mathbf{E}[\cdot]</script></span> 만큼 Shifting 해도 <span><script type="math/tex">\beta</script></span> 값은 변화가 없다는 것이다. 
> <div class="math"><script type="math/tex; mode=display">
> \beta = \frac{\mathbf{Cov}[R_b, R_p]}{\mathbf{Var} [R_b]} = \frac{\mathbf{Cov} \left[ R_b + \mathbf{E}[R_b], R_p + \mathbf{E}[R_p] \right]}{\mathbf{Var} [R_b + \mathbf{E}[R_b]]}
> </script></div>
> 이제 샘플 공분산과 샘플분산 방식으로 위의 분자/분모값을 각각 구해보자. 투자전략 수익률의 샘플평균 <span><script type="math/tex">{\bar r}_p \equiv \mathbf{E}[R_p]</script></span> 와 BM 수익률의 샘플평균 <span><script type="math/tex">{\bar r}_b \equiv \mathbf{E}[R_b]</script></span> 에 대해서, 
> <div class="math"><script type="math/tex; mode=display">
> \begin{aligned}
> \mathbf{Cov} \left[ R_b + \mathbf{E}[R_b], R_p + \mathbf{E}[R_p] \right] 
> &= \mathbf{Cov} \left[ R_b + {\bar r}_b, R_p + {\bar r}_p \right] \\
> &= \frac{1}{m} (\mathbf{r}_b - {\bar r}_b + {\bar r}_b)^\mathsf{T} (\mathbf{r}_p - {\bar r}_p + {\bar r}_p)
> \end{aligned}
> </script></div>
