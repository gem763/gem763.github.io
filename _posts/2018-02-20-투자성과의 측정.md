...
>
> 하지만 내 투자전략의 수익률이 정규분포를 따른다는 보장은 없다. 일반적으로 **주식** 등의 금융자산 수익률은, 정규분포가 아닌 **Fat-tail 분포**를 따른다고 알려져 있다. 따라서 해당 주식의 수익률이 특정 범주에 속할 가능성은 생각보다 낮을 가능성이 높다. 금융자산들 간의 포트폴리오인, 특정 투자전략의 경우도 마찬가지라고 생각된다. 즉 위 사례를 다시 보면, <span><script type="math/tex">\pm</script></span> 4%의 범위에 있을 가능성은 2/3 보다도 낮을 것이다. 
> 
...
<span><script type="math/tex">\beta</script></span>의 형태를 유도해보자. 우선 주요변수를 셋팅한다. 
* 투자전략의 수익률 변수 <span><script type="math/tex">R_p \in \mathbb{R}</script></span> 
* BM 수익률 변수 <span><script type="math/tex">R_b \in \mathbb{R}</script></span>
...
투자전략의 수익률과 BM 수익률 간의 상관계수가 <span><script type="math/tex">\rho \equiv \frac{\mathbf{Cov}[R_b, R_p]}{\sqrt{\mathbf{Var}[R_b] \mathbf{Var}[R_p]}}</script></span> 로 정의되므로, <span><script type="math/tex">\beta</script></span>는 다음과 같이 도출된다. 
...
<div class="math"><script type="math/tex; mode=display">
\hat\beta = \frac{\mathbf{Cov}[R_b, R_p]}{\mathbf{Var} [R_b]}  = \frac{\frac{1}{m} (\mathbf{r}_b - {\bar r}_b)^\mathsf{T} (\mathbf{r}_p - {\bar r}_p)}{\frac{1}{m} (\mathbf{r}_b - {\bar r}_b)^\mathsf{T} (\mathbf{r}_b - {\bar r}_b)} = (\mathbf{z}_b^\mathsf{T} \mathbf{z}_b)^{-1} \mathbf{z}_b^\mathsf{T} \mathbf{z}_p
</script></div>
...
우선 [최소자승법 (OLS: Ordinary Least Squares)](https://en.wikipedia.org/wiki/Ordinary_least_squares)으로 회귀계수를 구해보자. 각 수익률 변수의 기대값을 이용하여 좌표변환하면 <span><script type="math/tex">\alpha</script></span> 를 제거할 수 있다. 
...
따라서 결과적으로 **회귀선은 <span><script type="math/tex">(\mathbf{E}[R_b], \mathbf{E}[R_p])</script></span> 점을 지나가게 된다**. 그리고 위에서 정의한 샘플데이터 벡터 <span><script type="math/tex">\mathbf{z}_b</script></span> 및 <span><script type="math/tex">\mathbf{z}_p</script></span>에 대하여 다음과 같이 오차 샘플벡터 <span><script type="math/tex">\mathbf{e} \in \mathbb{R}^m</script></span>를 정의할 수 있다. 
...
따라서 회귀계수 <span><script type="math/tex">\beta</script></span>를 다음과 같이 구할 수 있다. [^min_err]

[^min_err]: 오차항의 제곱합을 최소화하는 방식으로 구할 수도 있다. [여기](https://en.wikipedia.org/wiki/Simple_linear_regression)를 참고. 
...
즉 결론은, **샘플링으로 추정한 <span><script type="math/tex">\beta</script></span>는 회귀계수로 산출한 <span><script type="math/tex">\beta</script></span>와 완전히 동일**한 값이다. 
...
## Consistency
**투자성과의 지속성**을 평가하기 위한 지표이다. 투자성과의 로그 누적수익률에 선형회귀분석을 시행한 후, [R-Square](https://en.wikipedia.org/wiki/Coefficient_of_determination)를 취한다. 결과적으로는 로그 누적수익률이 얼마나 선형(Linear)에 가까운 지를 측정하게 된다. 

<br/>

### R-Square
변수 <span><script type="math/tex">X, Y \in \mathbb{R}</script></span> 가 다음의 선형관계를 가지고 있다고 가정하자. 

<div class="math"><script type="math/tex; mode=display">
Y = \alpha + \beta X + \epsilon
</script></div>

여기서 <span><script type="math/tex">\epsilon \in \mathbb{R}</script></span> 은 다음의 성질을 가지고 있는 오차항 변수이다. 

* <span><script type="math/tex">\mathbf{E} [\epsilon] = 0</script></span>
* <span><script type="math/tex">\mathbf{Cov} [X, \epsilon] = 0</script></span>

위의 식에서 <span><script type="math/tex">\epsilon</script></span>을 제외한 나머지는 선형회귀식으로 설명할 수 있는 부분이므로, 이를 종속변수 <span><script type="math/tex">Y</script></span>의 추정값 <span><script type="math/tex">\hat Y</script></span> 로 놓는다. 

<div class="math"><script type="math/tex; mode=display">
\alpha + \beta X ~\overset{\text{let}}{=} ~\hat Y ~\Longrightarrow~ Y = \hat Y + \epsilon
</script></div>

오차항 변수의 성질에 의해 <span><script type="math/tex">\mathbf{Cov}[\hat Y, \epsilon]</script></span> <span><script type="math/tex">= \mathbf{Cov}[\alpha + \beta X, \epsilon]</script></span> <span><script type="math/tex">= \beta \mathbf{Cov} [X, \epsilon] = 0</script></span> 이므로 다음을 얻게 된다. 
<div class="math"><script type="math/tex; mode=display">
\begin{aligned}
\mathbf{Var}[Y] &= \mathbf{Var}[\hat Y + \epsilon] \\
&= \mathbf{Var}[\hat Y] + \mathbf{Var}[\epsilon] + 2 \mathbf{Cov} [\hat Y, \epsilon] \\
&= \mathbf{Var}[\hat Y] + \mathbf{Var}[\epsilon]
\end{aligned}
</script></div>

R-Square <span><script type="math/tex">\mathbf{R}^2</script></span> 는 다음과 같이 정의된다. 

<div class="math"><script type="math/tex; mode=display">
\mathbf{R}^2 \equiv \frac{\mathbf{Var}[\hat Y]}{\mathbf{Var}[Y]} = 1 - \frac{\mathbf{Var}[\epsilon]}{\mathbf{Var}[Y]}
</script></div>

즉 **R-Square는 종속변수의 분산 중 회귀모형으로 설명이 가능한 부분의 비율**을 의미한다고 볼 수 있다. **결정계수** (Coefficient of determination) 또는 **설명력**이라고도 한다. 

실전에서는 샘플분산을 통해 R-Square를 계산하게 된다. 다음의 각 샘플데이터에 대해서, 

* 종속변수 샘플 <span><script type="math/tex">\mathbf{y} = [y_i] \in \mathbb{R}^m</script></span>, <span><script type="math/tex">\bar y = \mathbf{E}[Y] = \frac{1}{m} \sum_i y_i</script></span>
* 추정값 샘플 <span><script type="math/tex">\hat\mathbf{y} = [\hat y_i] \in \mathbb{R}^m</script></span>, <span><script type="math/tex">\bar {\hat y} = \mathbf{E}[\hat Y] = \mathbf{E}[Y] = \bar y</script></span>
* 오차항 샘플 <span><script type="math/tex">\mathbf{e} \equiv \mathbf{y} - \hat\mathbf{y} = [e_i]</script></span>, <span><script type="math/tex">\bar e = \mathbf{E}[\epsilon] = 0</script></span> 

다음의 세 가지를 새로 정의하자. 

<div class="math"><script type="math/tex; mode=display">
\begin{aligned}
\small\it{\text{Total Sum of Squares}} ~~~&\mathbf{TSS} \equiv \textstyle \sum_i (y_i - \bar y)^2 = m \mathbf{Var}[Y]\\
\small\it{\text{Explained Sum of Squares}} ~~~&\mathbf{ESS} \equiv \textstyle \sum_i (\hat y_i - \bar y)^2 = m \mathbf{Var}[\hat Y]\\
\small\it{\text{Residual Sum of Squares}} ~~~&\mathbf{RSS} \equiv \textstyle \sum_i e_i^2 = m \mathbf{Var}[\epsilon]
\end{aligned}
</script></div>

<div class="math"><script type="math/tex; mode=display">
\underbrace{\mathbf{Var}[Y]}_{\normalsize\frac{\mathbf{TSS}}{m}} = \underbrace{\mathbf{Var}[\hat Y]}_{\normalsize\frac{\mathbf{ESS}}{m}} + \underbrace{\mathbf{Var}[\epsilon]}_{\normalsize\frac{\mathbf{RSS}}{m}}
</script></div>


따라서 R-Sqaure 는 다음과 같이 계산된다. 

<div class="math"><script type="math/tex; mode=display">
\mathbf{R}^2 = \frac{\mathbf{ESS}}{\mathbf{TSS}} = 1 - \frac{\mathbf{RSS}}{\mathbf{TSS}}
</script></div>
