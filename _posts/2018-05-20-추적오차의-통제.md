...
### Filtering 효과
우선 이 포스트에서 설계한 두 가지 Filter들의 효과를 확인해보자. GFAM에 Target Tracking error <span><script type="math/tex">\mathbf{T}</script></span>=3% 로 시뮬레이션 하였고, Smoothing Filter와 Short-term Alert Filter를 차례로 추가 적용하였다. BM은 **AGG** (US Aggregate bond ETF) 로 하였다. 
