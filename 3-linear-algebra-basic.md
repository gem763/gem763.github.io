...
* <span class="math">$\mathbf{C}_{i,j} = (-1)^{i+j} \mathbf{M}_{i,j}$</span> : 코팩터(Cofactor)
* <span class='math">$\mathbf{M}_{i,j}$</span> : 마이너(Minor)라고 하며, $\mathbf{A}$에서 $i$-행과 $j$-열을 지워서 얻어진 행렬의 행렬식
