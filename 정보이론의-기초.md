...
### 주요성질
1. <span><script type="math/tex">\mathbf{D}_{\text{KL}} (p \parallel q) \ne \mathbf{D}_{\text{KL}} (q \parallel p)</script></span>
2. <span><script type="math/tex">\mathbf{D}_{\text{KL}} (p \parallel q) \ge 0</script></span>
3. <span><script type="math/tex">\mathbf{D}_{\text{KL}} (p \parallel q) = 0  \Longleftrightarrow p=q</script></span>

여기서 2번 성질을 이용하면, KL 다이버전스의 정의에 의해 다음의 부등식을 추가적으로 알게 된다. 
...
**Proof.**
1번 성질은 자명하다. 2번과 3번을 증명해보자. <span><script type="math/tex">\log</script></span>는 [Strictly concave 함수](https://gem763.github.io/machine%20learning/Affinity%EC%99%80-Convexity.html#strictly-convex-function)이므로, [Jensen 부등식](https://gem763.github.io/machine%20learning/Affinity%EC%99%80-Convexity.html#jesen-%EB%B6%80%EB%93%B1%EC%8B%9D)을 이용하면, 
...
여기서는 <span><script type="math/tex">X</script></span>가 이산확률변수라고 가정(연속확률변수에 대해서도 같은 논리 적용가능)하였다. 
특히 등호는 <span><script type="math/tex">\frac{q(X)}{p(X)}</script></span>가 상수일 때 성립하는데, <span><script type="math/tex">\frac{q(X)}{p(X)} = c</script></span> 에서

<div class="math"><script type="math/tex; mode=display">
1 = \sum_x q(x) = c \sum_x p(x) = c
</script></div>

따라서 <span><script type="math/tex">p = q</script></span> 를 얻게 된다. 
